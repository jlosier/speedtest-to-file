...
try:
    ...
    print("Cannot run. Improper arguments supplied.\nExample1: "
          ...
          "$ python *.py true /home/pi/Documents/speedtest/speedtest.json")
    sys.exit()
...
def writeToJson(results, filePath):
    ...
    try:
        ...
        writeMessage(message="Results successfully written to json file",
                     verbose=verbose)
...
if ("Cannot" not in speed):
    ...
    writeMessage(message="There was a problem getting results from "
                         "speedtest-cli", verbose=verbose)
    location = ["ERROR"]
    ping = ["ERROR", "0"]
    download = ["ERROR", "0"]
    upload = ["ERROR", "0"]
...
writeMessage(message="Timestamp: " + results[0]['timestamp'],
             verbose=verbose)
writeMessage(message="Location: " + results[0]['location'],
             verbose=verbose)
writeMessage(message="Ping (ms): " + results[0]['ping'],
             verbose=verbose)
writeMessage(message="Download (Mbit/s): " + results[0]['download'],
             verbose=verbose)
writeMessage(message="Upload (Mbit/s): " + results[0]['upload'],
             verbose=verbose)
